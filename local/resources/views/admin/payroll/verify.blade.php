<form class="form-horizontal table-responsive" style="font-size: 12px" action="{{ route('payroll.verify') }}" method="post">
	...
	<div class="modal-footer">
		...
		<button type="button" name="verify" data-title="Confirm Verification" data-content="Are you sure you want to verify the loan application of {{ $loan->FullName }}?" class="btn btn-success btn-sm btnSave"><i class="fa fa-check-square-o"></i> Verify</button>
		<button type="button" name="deny" data-title="Deny Application" data-content="Are you sure you want to deny the loan application of  {{ $loan->FullName }}?" class="btn btn-danger btn-sm btnSave" ><i class="fa fa-thumbs-down"></i> Deny</button>
		@endif
